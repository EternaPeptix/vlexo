...
start_s2_exo() {
  ...
  "${SSH_S2[@]}" "jeweled@${s2_tb}" \
    "cd ${EXO_DIR} && export EXO_MEMORY_THRESHOLD=0.92 EXO_MACMON_PATH=/opt/homebrew/bin/macmon EXO_KV_BITS=8 EXO_KV_GROUP_SIZE=64 EXO_SPEC_PREFILL=1 EXO_BOOTSTRAP_PEERS='${bootstrap}' && ./.venv/bin/exo --fast-synch --libp2p-port ${LIBP2P_PORT_S2} >> ${LOG_DIR}/nohup.out 2>&1 & disown"
}
